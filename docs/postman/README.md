...
- `nca-toolkit.postman_collection.json` — 25 requests grouped into 9 folders. Each request has assertion scripts that check status, response shape, and performance.
- `nca-toolkit.postman_environment.json` — variables (`baseUrl`, `apiKey`, sample media URLs, optional `webhookUrl`, optional `gcpBucketName`). Override with your live values.
...
| `webhookUrl` | `https://webhook.site/abc-123` | Optional. Only used by the async-pattern folder. Grab a free URL at https://webhook.site. |
